...
// Common function to generate HTML for a chapter
function generateChapterHTML(chapter, displayStyle) {
  // Extract all chapter IDs from prov_regs and their nested chapters
  let chapterIds = [];

  //   console.log(chapter);

  chapter.prov_regs.forEach((provReg) => {
    // Add the provReg ID itself
    // chapterIds.push(provReg.id);

    // Add all chapter IDs from this province if they exist
    if (provReg.chapters && provReg.chapters.length > 0) {
      provReg.chapters.forEach((ch) => {
        chapterIds.push(ch.id);
      });
    }
  });

  const chapterIdList = [...new Set(chapterIds)].join(",");

  return `
    <div style="position: relative; display: inline-block;">
      <a href="viewmembers.html?list=${chapterIdList}&label=${chapter.region}">
        <button class="btn btn-region">${chapter.region}</button>
      </a>
      <button class="border-0 deleteChapter" data-id="${chapter.id}" data-label-name="${chapter.region}"
...
// Common function to process and display data
async function processAndDisplayData(
  ...
  const regionData = await getCachedData("/api/region");

  if (!regionData) return;

  let htmlContent = "";
  const filteredData = filterFn ? regionData.filter(filterFn) : regionData;
  ...
  if (document.getElementById(container)) {
    endpoint_url = url;
    sessionStorage.setItem("btnLabel", label);
...
//           Create ${
//             (sessionStorage.getItem("btnLabel") || "").charAt(0).toUpperCase() +
//             (sessionStorage.getItem("btnLabel") || "").slice(1)
//           }
//         </button>
//       </div>`;
